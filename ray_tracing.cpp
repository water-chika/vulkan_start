...
template <class T> class add_dispatch_loader : public T {
public:
  auto get_dispatch_loader() { return m_dld; }

private:
  vk::DispatchLoaderDynamic m_dld;
};

template <class T> class add_acceleration_structure_geometry : public T {
public:
...
template <class T> class add_accelration_structure_build_sizes : public T {
public:
  using parent = T;
...
template <class T> class add_acceleration_structure : public T {
public:
  using parent = T;
  ...
  ~add_acceleration_structure() {
    ...
    device.destroyAccelerationStructureKHR(m_acceleration_structure, nullptr,
                                           loader);
  }
...
template <class T> class build_acceleration_structure : public T {
public:
  using parent = T;
  build_acceleration_structure() {
    ...
    auto build_geometries = std::vector{
        vk::AccelerationStructureBuildGeometryInfoKHR{}.setGeometries(
            geometries)};
    auto build_ranges = std::array{
        vk::AccelerationStructureBuildRangeInfoKHR{}.setPrimitiveCount(1)};
...
using ray_tracing_app = add_acceleration_structure<add_dispatch_loader<add_buffer_as_member<set_buffer_size<
    100,
    set_buffer_usage<
        vk::BufferUsageFlagBits::
            eAccelerationStructureBuildInputReadOnlyKHR,
        add_command_buffer<add_command_pool<add_queue<add_recreate_surface_for_swapchain_images<
            add_swapchain_images<add_recreate_surface_for_swapchain<add_swapchain<add_swapchain_image_extent_equal_surface_current_extent<
                add_swapchain_image_format<add_device<add_swapchain_extension<add_empty_extensions<
                    add_find_properties<cache_physical_device_memory_properties<add_recreate_surface_for_cache_surface_capabilites<
                        cache_surface_capabilities<add_recreate_surface_for<test_physical_device_support_surface<
                            add_queue_family_index<add_physical_device<add_recreate_surface<
                                vulkan_windows_helper::add_windows_surface<add_instance<
                                    add_win32_surface_extension<add_surface_extension<add_empty_extensions<
                                        add_window<adjust_window_resolution<set_window_resolution<
                                            151, 151,
                                            set_window_style<
                                                WS_OVERLAPPEDWINDOW,
                                                add_window_class<add_window_process<
                                                    empty_class>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>;
